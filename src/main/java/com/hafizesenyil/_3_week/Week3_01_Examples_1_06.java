...
public class Week3_01_Examples_1_06 {
    ...
    // Iterative Metod
    // Recursive Metod ---> BU BÖLÜMÜN KONUSU
    // System
    // Access Modifier


    // Kullanicidan sayisal değerler al
    public static long isNumber() {
        // variable
        long number; // result= 1 vermeliyizki 1 sayısı çarpmada etkisizdir ve bşlangıç değeridir
    ...
    public static void iterativeFactoriyel(long userData) {
        long result = 1; // bu kısımda tekrar 1 ataması yaptık önceki değerleri ile işlem yapıp girmesin diye
        for (int i = 1; i <= userData; i++) {
            // result= result*i;
            result *= i;
        }
        System.out.println(SpecialColor.GREEN + userData + " İterative sayisinin faktöryeli : " + userData + "! = " + result + SpecialColor.RESET);

    }

    // Recursive Metod
    public static long recursiveFactoriyel(long userData) {
        // sıfır ve bir faktöryeli 1 dir
    ...
    public static void main(String[] args) {
        ...
        isResult();// kullanıcı sadece bu kısmı görmeli yani soyutlanmalı
    }
}// end class
